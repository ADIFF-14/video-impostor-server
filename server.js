...
io.on('connection', (socket) => {
    socket.on('unirse', (datos) => {
        ...
        if (nombre === 'anderson') {
            ...
            return; // Anderson NO entra a la lista de jugadores que votan
        }

        // Solo los hermanos entran aquí
    ...
    function notificarTurno() {
        if (indiceTurno < ordenHablar.length) {
            ...
            io.to('sala_proyeccion').emit('turnoEnPantalla', turno);
        } else {
    ...
    function procesarVotacion() {
        ...
        if (esImp) {
            ...
            io.to('sala_proyeccion').emit('resultadoFinalProyeccion', { titulo: "¡ATRAPADO!", sub: `${exp.nombre} era el Impostor`, palabra: palabraActual, color: "#00e676" });
        } else if (rondaActual >= 2) {
            io.emit('resultadoVotacion', { mensaje: "¡EL IMPOSTOR GANA!", terminar: true, palabraReal: palabraActual });
            io.to('sala_proyeccion').emit('resultadoFinalProyeccion', { titulo: "¡IMPOSTOR GANA!", sub: "No fue descubierto", palabra: palabraActual, color: "#ff4444" });
        } else {
            rondaActual++; votosRecibidos = {};
            io.emit('resultadoVotacion', { mensaje: "Inocente expulsado...", terminar: false });
            io.to('sala_proyeccion').emit('resultadoFinalProyeccion', { titulo: "ERA INOCENTE", sub: `${exp?.nombre} fuera`, temporal: true, color: "#ff4444" });
