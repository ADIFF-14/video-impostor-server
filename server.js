...
// Lista de 50 temas bíblicos (puedes añadir más)
const palabrasBiblicas = ["El Arca de Noé", "Los Diez Mandamientos", "El Mar Rojo", "La Zarza Ardiendo", "David y Goliat", "El Jardín del Edén", "El Maná", "La Torre de Babel", "La Túnica de José", "El Pez de Jonás", "Las Murallas de Jericó", "Sanson y Dalila", "El Rey Salomón", "La Reina Ester", "La Estrella de Belén", "El Pesebre", "El Bautismo", "El Rio Jordán", "La Ultima Cena", "La Corona de Espinas", "La Cruz", "La Tumba Vacía", "La Resurrección", "El Espíritu Santo", "La Armadura de Dios"];
...
io.on('connection', (socket) => {
    socket.on('unirse', (datos) => {
        ...
        if (nombre === 'anderson') {
            ...
            return; // Anderson no cuenta como jugador para votar
        }
    ...
    function iniciarDebate() {
        ...
        // REGLA: El impostor nunca empieza hablando
        if (ordenHablar[0] && ordenHablar[0].rol === "IMPOSTOR") {
            ordenHablar.push(ordenHablar.shift());
        }
        notificarTurno();
    }
    ...
    function procesarVotacion() {
        ...
        if (fueImpostor) {
            ...
            io.to('sala_proyeccion').emit('resultadoFinalProyeccion', { 
                titulo: "¡TE ATRAPAMOS!", 
                sub: `${expulsado.nombre} era el Impostor`, 
            ...
            io.to('sala_proyeccion').emit('resultadoFinalProyeccion', { 
                titulo: "¡EL IMPOSTOR GANA!", 
                sub: `No pudieron descubrirlo. El impostor era: ${imp ? imp.nombre : '??'}`, 
                palabra: palabraActual, 
                color: "#ff4444" 
            });
        } else {
            ...
            io.to('sala_proyeccion').emit('resultadoFinalProyeccion', { 
                titulo: "INOCENTE", 
                sub: `${expulsado ? expulsado.nombre : 'Alguien'} no era el impostor`, 
                temporal: true, 
                color: "#ff4444" 
            });
